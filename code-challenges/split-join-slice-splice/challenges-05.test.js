...
const listFoods = (recipe) => {
    ...
    recipe.ingredients.forEach(function (value) {
        let frstIndx = value.indexOf(' ');
        let newValue = value.slice(frstIndx + 1);
        let scndIndx = newValue.indexOf(' ');
        newValue = newValue.slice(scndIndx + 1);
...
const splitFoods = (recipe) => {
    ...
    recipe.ingredients.forEach(function (value) {
        ...
        for (let i = 2; i < newArr.length; i++) {
            ingArr.push(newArr[i]);
        }
        // console.log(ingArr.join(' '));
        result.push(ingArr.join(' '));
    });
    //   console.log(result);
    return result;
};
...
const stepActions = (recipe) => {
    let result = [];
    // Solution code here...
    recipe.steps.forEach(function (value) {
...
const removeLastCharacters = (str, numberOfCharacters) => {
    ...
    if (numberOfCharacters < 0) {
        ...
        //   console.log(str.replace(str.slice(-1 * numberOfCharacters), ''));
        return str.replace(str.slice(-1 * numberOfCharacters), '')
    }
};
...
const extractVowels = (str) => {
    ...
    for (let i = 0; i < strArr.length; i++) {
        for (let k = 0; k < strArr[i].length; k++) {
            for (let j = 0; j < vowels.length; j++) {
                if (strArr[i][k] === vowels[j]) {
                    removedVowels = removedVowels + `${vowels[j]}`;
                    strArr[i] = strArr[i].replace(vowels[j], '');
                    k = k - 1;
                }
            }
        }
    ...
    let strNew = strArr.join(' ');
    let removedVowelsArr = removedVowels.split('');
    removedVowelsArr.sort(function (a, b) {
        return a > b;
    })
    removedVowels = removedVowelsArr.join('');
    result.push(strNew, removedVowels);
    return result;
};
