...
const count = (target, input) => {
    // Solution code here...
    let count = input.reduce((acc, arr) => {
        arr.map((num) => {
            num === target ? acc = acc + 1 : acc = acc + 0;
        })
        return acc;
    }, 0)
    return count;
...
const divisibleByFiveTwoToThePower = (input) => {
    ...
    let result = newArr.map(arr => {
        let filtered = arr.map(num => {
...
let findShortest = (data) => {
    // Solution code here...
    let height = 1000;
    let shorter = data.reduce((acc, obj) => {
        if (Number(obj.height) < height){
