...
const countNumberOfElements = (arr) => {
    // Solution code here...
    let numElements = arr.reduce((acc, num) => {
        return acc = acc + 1;
    }, 0)
    return numElements;
};
...
const reversedString = (str) => {
    ...
    let reversed = strArr.reduce((acc, char) => {
        return char + acc;
    }, '')
    return reversed;
};
...
const countNumberOfChildren = (arr) => {
    // Solution code here...
    let childNum = arr.reduce((acc, val) => {
...
const calculateAverage = (arr) => {
    // Solution code here...
    let averObj = arr.reduce((acc, num) => {
         acc['count'] = acc.count + 1;
         acc['sum'] = acc.sum + num;
         return acc;
    },{'count': 0, 'sum': 0})
    // console.log(averObj);
    return averObj.sum / averObj.count;
};
...
const extractStat = (statName, arr) => {
    // Solution code here...
    let result = arr.reduce((matched, stat) => {
        // console.log(stat.stat.name)
        if (stat.stat.name === statName){
            matched = stat;
        }
        else if(stat.name === null){
            matched = null;
        }
